...
var modalV2 = {
  create(id, title, inputs) {
    ...
    inputs.forEach((input) => {
      const textInput = new discord_js.TextInputBuilder().setCustomId(input.id).setLabel(input.label).setStyle(input.style || discord_js.TextInputStyle.Short).setRequired(input.required !== false);
      ...
      const actionRow = new discord_js.ActionRowBuilder().addComponents(textInput);
  ...
  contact(id) {
    return this.create(id, "Contact Form", [
      ...
      { id: "message", label: "Message", placeholder: "Tell us more...", required: true, style: discord_js.TextInputStyle.Paragraph }
    ]);
  },
  feedback(id) {
    return this.create(id, "Feedback Form", [
      { id: "rating", label: "Rating (1-10)", placeholder: "Rate your experience", required: true },
      { id: "feedback", label: "Feedback", placeholder: "Share your thoughts...", required: true, style: discord_js.TextInputStyle.Paragraph },
      { id: "suggestions", label: "Suggestions", placeholder: "Any suggestions for improvement?", style: discord_js.TextInputStyle.Paragraph }
    ]);
  },
  settings(id, fields) {
    ...
    if (this.textComponents.length > 0) {
      ...
      this.textComponents.forEach((text) => {
        container.addTextDisplayComponents(
          new discord_js.TextDisplayBuilder().setContent(text)
        );
      });
...
var SimpleEmbedImpl = class {
  ...
  build() {
    ...
    if (this.embedTitle) {
      container.addTextDisplayComponents(
        new discord_js.TextDisplayBuilder().setContent(`**${this.embedTitle}**`)
      );
    }
    if (this.embedDescription) {
      container.addTextDisplayComponents(
        new discord_js.TextDisplayBuilder().setContent(this.embedDescription)
      );
    }
...
${field.value}`;
        container.addTextDisplayComponents(
          new discord_js.TextDisplayBuilder().setContent(fieldText)
        );
      });
    }
    if (this.embedFooter) {
      container.addTextDisplayComponents(
        new discord_js.TextDisplayBuilder().setContent(`*${this.embedFooter}*`)
      );
    }
    if (this.embedTimestamp) {
      container.addTextDisplayComponents(
        new discord_js.TextDisplayBuilder().setContent(`<t:${Math.floor(this.embedTimestamp.getTime() / 1e3)}:R>`)
      );
    }
    components.push(container.toJSON());
    if (this.embedThumbnail) {
      const thumbContainer = new discord_js.ContainerBuilder();
      thumbContainer.addTextDisplayComponents(
        new discord_js.TextDisplayBuilder().setContent(`![Thumbnail](${this.embedThumbnail})`)
      );
      components.push(thumbContainer.toJSON());
    }
    if (this.embedImage) {
      const imageContainer = new discord_js.ContainerBuilder();
      imageContainer.addTextDisplayComponents(
        new discord_js.TextDisplayBuilder().setContent(`![Embed image](${this.embedImage})`)
      );
...
var btn = {
   ...
   */
  success(id, label) {
    return new discord_js.ButtonBuilder().setCustomId(id).setLabel(label).setStyle(discord_js.ButtonStyle.Success);
  },
  /**
...
var select = {
  ...
  string(id, placeholder, options) {
    const menu = new discord_js.StringSelectMenuBuilder().setCustomId(id).setPlaceholder(placeholder);
    options.forEach((option) => {
      const menuOption = new discord_js.StringSelectMenuOptionBuilder().setLabel(option.label).setValue(option.value);
   ...
   */
  user(id, placeholder) {
    return new discord_js.UserSelectMenuBuilder().setCustomId(id).setPlaceholder(placeholder);
  },
  /**
   ...
   */
  role(id, placeholder) {
    return new discord_js.RoleSelectMenuBuilder().setCustomId(id).setPlaceholder(placeholder);
  },
  /**
  ...
  channel(id, placeholder) {
    return new discord_js.ChannelSelectMenuBuilder().setCustomId(id).setPlaceholder(placeholder);
  }
};
...
${text}`);
  ...
  new discord_js.ActionRowBuilder().addComponents(
...
function autoShard(token, options = {}) {
  ...
  const manager = new discord_js.ShardingManager("./dist/bot.js", {
    token,
    totalShards,
    respawn
  });
...
exports.broadcastToShards = broadcastToShards;
exports.btn = btn;
...
exports.getTotalGuildCount = getTotalGuildCount;
...
exports.migrateEmbeds = migrateEmbeds;
